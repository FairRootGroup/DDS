...
void DDSTopology::ParsePropertyTree(const ptree& _pt)
{
    ...
    try {
        ...
        for (auto it = pt.begin(); it != pt.end(); it++) {
            ...
            if (v.first == "task") ParsePropertyTreeTask(v.second);
            else if (v.first == "socket") ParsePropertyTreeSocket(v.second);
            else if (v.first == "collection") ParsePropertyTreeCollection(v.second);
            else if (v.first == "group") ParsePropertyTreeGroup(v.second);
        }
...
void DDSTopology::ParsePropertyTreeCollection(const ptree& _pt)
{
    ...
    for_each(tasks.begin(), tasks.end(), [] (const std::string& _v) {std::cout << _v << " "; });
    std::cout << std::endl;
...
void DDSTopology::ParsePropertyTreeGroup(const ptree& _pt)
{
    ...
    std::cout << "group: " << name << " ";
    for_each(collections.begin(), collections.end(), [] (const std::string& _v) {std::cout << _v << " "; });
    std::cout << std::endl;
...
void DDSTopology::PrintPropertyTree(
