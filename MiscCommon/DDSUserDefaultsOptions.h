...
namespace DDS
{
    ...
    //        bool m_genTempSSHKeys; //!< Indicates, whether PoD need to generate temporary SSH keys for WNs. Useful in case PoD server has only ssh port open
    // for
    //                               //incoming connection.
    //    } SServerOptions_t;
    //
    ...
    class CDDSUserDefaults
    {
        ...
        //                "server.agent_ports_range_min", boost::program_options::value<unsigned
        // int>(&m_options.m_server.m_agentPortsRangeMin)->default_value(22001))(
        //                "server.agent_ports_range_max", boost::program_options::value<unsigned
        // int>(&m_options.m_server.m_agentPortsRangeMax)->default_value(23000))(
        ...
        //            config_file_options.add_options()(
        //                "worker.work_dir",
        // boost::program_options::value<std::string>(&m_options.m_worker.m_common.m_workDir)->default_value("$POD_LOCATION/"), "")(
        //                "worker.logfile_dir",
        ...
        //                arg[0] = new char[1];
        //                arg[0][0] = '\0';
        //                boost::program_options::store(boost::program_options::basic_command_line_parser<char>(1, arg).options(config_file_options).run(),
        // m_keys);
        //                delete[] arg[0];
        //            }
        //
    ...
    struct SDDSSSHOptions
    {
        ...
        void load(const std::string& _filename)
        {
            ...
            {
                read_ini(_filename, pt);
                m_config = pt.get<std::string>("dds-ssh.config");
            }
            catch (...)
            {
                // ignore missing nodes
        ...
        void save(const std::string& _filename)
        {
            ...
            pt.put("dds-ssh.config", m_config);
