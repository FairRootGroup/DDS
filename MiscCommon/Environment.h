...
namespace dds::misc
{
    namespace impl
    {
                                ...
                                const std::string& _defaultDir)
        {
            ...
            {
                LOG(MiscCommon::info) << "DDS " << std::quoted(_dirName) << " directory (" << std::quoted(install)
    ...
    inline void setupEnv()
    {
        ...
        if (location != kDDSInstallPrefix)
        {
            if (location.empty())
            {
                ...
                LOG(MiscCommon::info) << "Set $DDS_LOCATION to " << std::quoted(location);
            }
            else
            {
                LOG(MiscCommon::info) << "$DDS_LOCATION (" << std::quoted(location) << ") differs from the linked one ("
