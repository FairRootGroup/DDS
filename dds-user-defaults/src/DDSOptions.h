...
namespace dds
{
    typedef struct SDDSGeneralOptions
    {
        ...
        unsigned int
            m_ddsCommanderPortRangeMin; //!< commander port range min value (should be open for incoming connection)
        unsigned int
            m_ddsCommanderPortRangeMax;  //!< commander port range max value (should be open for incoming connection)
