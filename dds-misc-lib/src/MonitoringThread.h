    ...
    class CMonitoringThread
    {
                ...
                [this, &_idleCallback, _idleTime]()
                {
                    try
                    {
                        ...
                        while (true)
                        {
                            ...
                            {
                                LOG(dds::misc::error) << "MonitoringThread exception on custom actions: " << _e.what();
                            }
                            catch (...)
                            {
                            ...
                            if (idleTime.count() > _idleTime)
                            {
                                ...
                                LOG(dds::misc::info) << "The process is idle for " << idleTime.count()
                                ...
                                // Call terminate
                                LOG(dds::misc::info) << "Sending SIGTERM to this process...";
                                std::raise(SIGTERM);
                                std::this_thread::sleep_for(WAITING_TIME);

                                // Kill process
                                LOG(dds::misc::info) << "The process still exists. Killing the process...";
                                killProcess();
                            }
      ...
      private:
        void killProcess()
        {
            ...
            if (pidToKill > 0 && dds::misc::IsProcessRunning(pidToKill))
            {
                LOG(dds::misc::log_stdout) << " self exiting (" << pidToKill << ")...";
                ...
                while (iter <= max_iter)
                {
                    if (!dds::misc::IsProcessRunning(pidToKill))
                    {
                        LOG(dds::misc::log_stdout) << std::endl;
                        break;
                    }
                    LOG(dds::misc::log_stdout) << ".";
                    sleep(1); // sleeping for 1 second
                    ++iter;
                }
                if (dds::misc::IsProcessRunning(pidToKill))
                {
                    LOG(dds::misc::error) << "FAILED to close the process.";
                    LOG(dds::misc::warning) << "Sending unconditional terminate to (" << pidToKill << ")...";
