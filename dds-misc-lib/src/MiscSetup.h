...
namespace dds::misc
{
                                ...
                                std::function<bool(int argc, char* argv[], Options_t*)> _parseFunc)
    {
        try
        {
            ...
            dds::misc::Logger::instance().init();         // Initialize log
            ...
            LOG(dds::misc::info) << "Starting with arguments: " << ss.str();
        ...
        {
            LOG(dds::misc::log_stderr) << e.what();
            return EXIT_FAILURE;
        }
        return EXIT_SUCCESS;
    }
    ...
    inline int defaultExecReinit(const boost::uuids::uuid& _sid)
    {
        try
        {
            ...
            dds::misc::Logger::instance().reinit();
        }
        catch (std::exception& e)
        {
            LOG(dds::misc::log_stderr) << e.what();
